...
MenuItem menu_items[] = {
	...
	{ "Display dynamic relocation table",
	  display_dynamic_relocation_table },
...
static Elf64_Half emit_ei_osabi(const Elf64_Ehdr *ehdr)
{
	...
	if (elf_ei_osabi >= ELFOSABI_SOLARIS &&
	    elf_ei_osabi <= ELFOSABI_OPENBSD)
		elf_ei_osabi -= 2;
...
static void print_elf_header(const Elf64_Ehdr *ehdr)
{
	...
	buf_ptr += snprintf(
		...
		ehdr->e_version, elf_e_version_id[ehdr->e_version],
		(int)ehdr->e_entry, (int)ehdr->e_phoff, (int)ehdr->e_shoff,
		(int)ehdr->e_flags, (int)ehdr->e_ehsize, (int)ehdr->e_phentsize,
		(int)ehdr->e_phnum, (int)ehdr->e_shentsize, (int)ehdr->e_shnum,
		(int)ehdr->e_shstrndx);

	elfprint(buffer);
}

static int display_elf_header(void *v)
{
	FileContents *filecontents = (FileContents *)v;

	Elf64_Ehdr ehdr = { 0 };
	if (get_elf_header(filecontents->buffer, filecontents->length, &ehdr) !=
	    0) {
...
static int display_program_header_table(void *v)
{
	FileContents *filecontents = (FileContents *)v;
	...
	for (int i = 0; i < ehdr.e_phnum; i++) {
		...
		if (phdr[i].p_type == PT_INTERP) {
			...
			memcpy(interp_path,
			       filecontents->buffer + phdr[i].p_offset,
			       phdr[i].p_filesz);
...
static void print_section_header(const Elf64_Shdr *section,
				 const Elf64_Ehdr *ehdr, const char *data)
{
	...
	for (int i = 0; i < ehdr->e_shnum; i++) {
		format_and_print(
			...
			section[i].sh_addr, 16, section[i].sh_offset, nrsz, spc,
			...
			section[i].sh_link, section[i].sh_info,
...
static int display_section_header_table(void *v)
{
	FileContents *filecontents = (FileContents *)v;
...
static int disassemble_code_section(void *v)
{
	print_and_wait("Disassemble code section\n");
	return 0;
}

static int display_symbol_table(void *v)
{
	print_and_wait("Display symbol table\n");
	return 0;
}

static int display_relocation_table(void *v)
{
	print_and_wait("Display relocation table\n");
	return 0;
}

static int display_dynamic_symbol_table(void *v)
{
	print_and_wait("Display dynamic symbol table\n");
	return 0;
}

static int display_dynamic_relocation_table(void *v)
{
	print_and_wait("Display dynamic relocation table\n");
	return 0;
}

static int display_dynamic_section(void *v)
{
	print_and_wait("Display dynamic section\n");
	return 0;
}

static int display_string_table(void *v)
{
	print_and_wait("Display string table\n");
	return 0;
}

static int display_all(void *v)
{
	print_and_wait("Display all\n");
	return 0;
}

static int exit_program(void *v)
{
