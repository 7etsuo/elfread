...
extern int elf_getshnum(Elf *__elf, size_t *__dst) __deprecated_attribute__;
...
extern int elf_getshstrndx(Elf *__elf, size_t *__dst) __deprecated_attribute__;
...
extern Elf_Data *elf_getdata_rawchunk(Elf *__elf, int64_t __offset,
				      size_t __size, Elf_Type __type);
...
/* Compute hash value.  */
extern unsigned long int elf_hash(const char *__string) __pure_attribute__;

/* Compute hash value using the GNU-specific hash function.  */
extern unsigned long int elf_gnu_hash(const char *__string) __pure_attribute__;
