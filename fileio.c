...
int robust_fseek(FILE *stream, long offset, int whence)
{
	if (fseek(stream, offset, whence) != 0) {
		UNUSED(fprintf(stderr, "Failed to seek in file.\n"));
		return -1;
	}
	return 0;
}
...
long robust_ftell(FILE *stream)
{
	...
	if (position == -1) {
		UNUSED(fprintf(stderr, "Failed to get file position.\n"));
		return -1;
	}
	return position;
}
...
void *robust_malloc(size_t size)
{
	...
	if (memory == NULL) {
		UNUSED(fprintf(stderr, "Failed to allocate memory.\n"));
	}
	return memory;
}
...
{
	if (ptr == NULL) {
		UNUSED(fprintf(stderr, "Attempted to free NULL pointer.\n"));
		return;
	}
	free(ptr);
}
...
FILE *robust_fopen(const char *filename, const char *mode)
{
	if (strlen(filename) > PATH_MAX) {
		UNUSED(fprintf(stderr, "Filename is too long.\n"));
		return NULL;
	}

	FILE *file = fopen(filename, mode);
	if (file == NULL) {
		UNUSED(fprintf(stderr, "Failed to open file.\n"));
		return NULL;
	}
	return file;
}
...
{
	if (fclose(stream) != 0) {
		UNUSED(fprintf(stderr, "Failed to close file.\n"));
		return -1;
	}
	return 0;
}
...
size_t robust_fread(void *ptr, size_t size, size_t nmemb, FILE *stream)
{
	...
	if (read_length != nmemb) {
		UNUSED(fprintf(stderr, "Failed to read file.\n"));
		return -1;
	}
	return read_length;
}

FileContents *allocate_buffer_for_file(FILE *file, FileContents *file_contents)
{
...
FILE *robust_fopen_secure(const char *filename, const char *mode)
{
	...
	return robust_fopen(filename, mode);
}
...
FileContents *robust_read_file(const char *filename)
{
	...
	size_t read_length = robust_fread(file_contents->buffer, 1,
					  file_contents->length, file);
	if (read_length == -1) {
		goto err;
	}
...
err:
	if (file_contents->buffer != NULL) {
		robust_free(file_contents->buffer);
	}

	if (file_contents != NULL) {
		robust_free(file_contents);
	}
