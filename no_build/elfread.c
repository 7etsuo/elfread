...
const char *g_help_menu = {"Usage: elfread <option(s)> elf-file(s)\n"
...
void display_elf_s_section_header(const Elf64_Shdr *section, const Elf64_Ehdr *ehdr, const void *data);
void display_elf_p_segment_header(const Elf64_Phdr *segment, const Elf64_Ehdr *ehdr, const void *data);
...
int get_string_table(const char *stringtable[], Elf64_Off stroff, Elf64_Xword sh_sz, const void *data);
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        static struct option long_options[] = {
            {"file-header", no_argument, 0, 'h'}, {"program-headers", no_argument, 0, 'l'},
            {"segments", no_argument, 0, 'l'},    {"section-headers", no_argument, 0, 'S'},
            {"sections", no_argument, 0, 'S'},    {"headers", no_argument, 0, 'e'},
...
void display_elf_s_section_header(const Elf64_Shdr *section, const Elf64_Ehdr *ehdr, const void *data)
{
    ...
    printf("There are %d section headers, starting at offset 0x%.4x:\n"
           "\n"
           "Section Headers:\n"
    ...
    for (int i = 0; i < ehdr->e_shnum; i++)
    {
        printf("[%*d] %-*s %-*s %-.*x %-.*x\n"
               ...
               elf_s_type_id[get_s_type_index(section[i].sh_type)], 16, section[i].sh_addr, 16, section[i].sh_offset,
               nrsz, spc, 16, section[i].sh_size, 16, section[i].sh_entsize,
               (section[i].sh_flags & SHF_WRITE ? 'W' : ' '), (section[i].sh_flags & SHF_ALLOC ? 'A' : ' '),
               (section[i].sh_flags & SHF_EXECINSTR ? 'X' : ' '), (section[i].sh_flags & SHF_MERGE ? 'M' : ' '),
               (section[i].sh_flags & SHF_STRINGS ? 'S' : ' '), (section[i].sh_flags & SHF_INFO_LINK ? 'I' : ' '),
               (section[i].sh_flags & SHF_LINK_ORDER ? 'L' : ' '),
               (section[i].sh_flags & SHF_OS_NONCONFORMING ? 'O' : ' '), (section[i].sh_flags & SHF_GROUP ? 'G' : ' '),
               (section[i].sh_flags & SHF_TLS ? 'T' : ' '), (section[i].sh_flags & SHF_COMPRESSED ? 'C' : ' '),
               (section[i].sh_flags & (1 << 12) ? 'x' : ' '), (section[i].sh_flags & SHF_MASKOS ? 'o' : ' '),
               (section[i].sh_flags & SHF_EXCLUDE ? 'E' : ' '), (section[i].sh_flags & SHF_ORDERED ? 'l' : ' '),
               (section[i].sh_flags & SHF_MASKPROC ? 'p' : ' '), section[i].sh_link, section[i].sh_info,
...
void display_elf_p_segment_header(const Elf64_Phdr *segment, const Elf64_Ehdr *ehdr, const void *data)
{
    ...
    printf("\nElf file type is %s\n"
           ...
           "\n",
           elf_e_type_id[elf_e_type], (int)ehdr->e_entry, (int)ehdr->e_phnum, (int)ehdr->e_phentsize);
    ...
    for (size_t i = 0; i < ehdr->e_phnum; i++)
    {
        ...
        printf("  %-12s\t 0x%.16x 0x%.16x 0x%.16x\n"
               ...
               elf_p_type_id[p_type_offs], segment[i].p_offset, segment[i].p_vaddr, segment[i].p_paddr,
               segment[i].p_filesz, segment[i].p_memsz, (segment[i].p_flags & PF_R ? 'R' : ' '),
               (segment[i].p_flags & PF_W ? 'W' : ' '), (segment[i].p_flags & PF_X ? 'E' : ' '), segment[i].p_align);
...
Elf64_Word emit_e_version(const Elf64_Ehdr *ehdr)
{
    Elf64_Word elf_e_version = ehdr->e_version != EV_CURRENT ? EV_NONE : ehdr->e_version;
...
void display_elf_header(const Elf64_Ehdr *ehdr)
{
    ...
    printf("  Class:                               %s\n"
           ...
           elf_class_id[elf_ei_class], elf_data_id[elf_ei_data], (int)ehdr->e_ident[EI_VERSION],
           elf_osabi_id[elf_ei_osabi], (int)ehdr->e_ident[EI_ABIVERSION], elf_e_type_id[elf_e_type],
           ehdr->e_machine >= EM_NUM ? "special\n" : elf_e_machine_id[ehdr->e_machine], ehdr->e_version,
           elf_e_version_id[elf_e_version], (int)ehdr->e_entry, (int)ehdr->e_phoff, (int)ehdr->e_shoff,
           (int)ehdr->e_flags, (int)ehdr->e_ehsize, (int)ehdr->e_phentsize, (int)ehdr->e_phnum, (int)ehdr->e_shentsize,
