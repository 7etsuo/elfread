...
int get_elf_phdr(void *buffer, off_t offset, Elf64_Ehdr *ehdr, Elf64_Phdr *phdr)
{
	...
	if (memcpy(phdr, (char *)buffer + ehdr->e_phoff + offset,
		   ehdr->e_phentsize) == NULL) {
...
int get_elf_shdr(void *buffer, off_t offset, Elf64_Ehdr *ehdr, Elf64_Shdr *shdr)
{
	if (buffer == NULL) {
		fprintf(stderr, "Buffer is NULL.\n");
		return -1;
	...
	if (memcpy(shdr, (char *)buffer + ehdr->e_shoff + offset,
		   ehdr->e_shentsize * ehdr->e_shnum) == NULL) {
